...
# 3. Create Supervisor
Write-Host "3. Creating Supervisor 'supervisor@pitayacode.io'..."
$supervisorBody = @{
    email    = "supervisor@pitayacode.io"
    password = "pitaya123"
...
$supLogin = Invoke-RestMethod -Uri "http://localhost:3008/auth/login" -Method Post -ContentType "application/json" -Body '{"email": "supervisor@pitayacode.io", "password": "pitaya123"}'
...
Write-Host "5. Creating Operator 'operator@pitayacode.io' as Supervisor..."
$opBody = @{
    email    = "operator@pitayacode.io"
    password = "pitaya123"
...
if ($count -ge 2) {
    # Should see themselves and the operator
